...
Convertor.PBRConvert(source_dir=packinfo["source_dir"], **config_convertor["pbr"]).convert(
    trims_compat = config_convertor["pbr"].get("trims_compat", False), 
    color_palettes_dir = config_convertor["trims"].get("color_palettes_dir")
)

Convertor.TrimsConvert(source_dir=packinfo["source_dir"], **config_convertor["trims"]).convert()

for file_path in Path(rp.get_export_dir()).glob("MC_1.21/assets/minecraft/textures/trims/**/*_resin_*.png"):
    file_path.unlink(missing_ok=True)

for file_path in Path(rp.get_export_dir()).glob("MC_1.21/assets/minecraft/textures/trims/**/*_copper_darker_*.png"):
    file_path.unlink(missing_ok=True)
